...
window.addEventListener("load", function(){
  ...
  for(let i = 0; i < partes_da_lista.length; i++){
    ...
    partes_da_lista[i].addEventListener("mousedown", function(evento){
      const texto = partes_da_lista[i].querySelectorAll("span")[0].innerText;
  ...
  /* Dicas de ferramenta (tooltips) das opções da lista */
  const links_editar_pessoa = document.getElementsByClassName("link_editar_pessoa");
  const links_excluir_pessoa = document.getElementsByClassName("link_excluir_pessoa");
  
  var opcoes = {
    "customClass": "tooltip_link_da_opcao_da_lista",
    "placement": "top"
  };
  for(let i = 0; i < links_editar_pessoa.length; i++){
    links_editar_pessoa[i].bs_tooltip_link_editar = new Tooltip(links_editar_pessoa[i], opcoes);
  }
