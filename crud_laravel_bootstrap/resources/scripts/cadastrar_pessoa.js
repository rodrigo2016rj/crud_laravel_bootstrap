import {Tooltip} from "alias_bootstrap/dist/js/bootstrap.min.js";
import "alias_bootstrap-icons/icons/calendar3.svg";
...
window.addEventListener("load", function(){
  ...
  function mostrar_calendario(){
    if(div_calendario.classList.contains("tag_oculta")){
      ...
      const largura_do_campo = parseInt(estilo_computado.width, 10);
      posicao_x += largura_do_campo / 2;
      posicao_x -= largura_do_calendario / 2;
      
      const altura_do_campo = parseInt(estilo_computado.height, 10);
      posicao_y += altura_do_campo;
  ...
  let ocultar_tooltip_descricao_do_setor = true;
  
  let bs_tooltip_descricao_do_setor = null;
  ...
  caixa_de_selecao_setor.addEventListener("change", function(){
    ...
    if(isNaN(id_do_setor) || id_do_setor % 1 != 0 || id_do_setor <= 0){
      if(bs_tooltip_descricao_do_setor !== null){
        bs_tooltip_descricao_do_setor.dispose();
        bs_tooltip_descricao_do_setor = null;
      }
      return;
    }
    
    const div_descricao_do_setor = document.getElementById("div_descricao_do_setor_id_" + id_do_setor);
    const texto = div_descricao_do_setor.innerText.trim();
    ...
    var opcoes = {
      "customClass": "tooltip_descricao_do_setor",
      "title": texto,
      "placement": "right",
      "trigger": "manual"
    };
    bs_tooltip_descricao_do_setor = new Tooltip(caixa_de_selecao_setor, opcoes);
    bs_tooltip_descricao_do_setor.show();
    
    const aria_describedby = caixa_de_selecao_setor.getAttribute("aria-describedby");
    const tooltip_descricao_do_setor = document.getElementById(aria_describedby);
    if(tooltip_descricao_do_setor !== null){
      tooltip_descricao_do_setor.removeEventListener("click", evento_clique_do_tooltip_descricao_do_setor);
      tooltip_descricao_do_setor.addEventListener("click", evento_clique_do_tooltip_descricao_do_setor);
    }
  });
  
  function evento_clique_do_tooltip_descricao_do_setor(){
    ocultar_tooltip_descricao_do_setor = false;
  }
  ...
  document.addEventListener("click", function(){
    ...
    if(ocultar_tooltip_descricao_do_setor){
      if(bs_tooltip_descricao_do_setor !== null){
        bs_tooltip_descricao_do_setor.dispose();
        bs_tooltip_descricao_do_setor = null;
      }
    }else{
      ocultar_tooltip_descricao_do_setor = true;
    }
  });
});
